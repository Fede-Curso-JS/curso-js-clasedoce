...
function addTask() {
    ...
    // -------------------------------------------------------------- CLASE 12 - OPERADORES TERNARIOS / AND / OR --------------------------------------------------------------

    // Usando operadores ternarios // and // or, funciona lo mismo con un if else, pero se puede poner todo el código que está arriba de este usando este tipo de operadores, o cual es realmente su funcionamiento?
    (TaskOne._task) && (TaskOne._project) ? console.log(`El nombre de tu tarea es ${TaskOne._task} para el proyecto de ${TaskOne._project}`) : console.log(`¡No es posible encontrar tu tarea y projecto solicidato!`);

    (TaskOne._time) || (TaskOne._price) ? console.log(`Trabajaste ${TaskOne._time}hs en tu tarea con un total de ${TaskOne._price}U$D la hora.`) : console.log(`¡No es posible encontrar tu tiempo y precio solicidato!`);

    // Desustructurando un objeto
    const totalTareas = {
        cantidad: 10,
        proyectos: 1
    }

    const desestructurar = (i) => {
        const {cantidad, proyectos} = i
    }

    desestructurar(totalTareas);

    // Buscamos la tarea en la que más horas haya invertido para generar un gráfico progresivo de tareas realizadas para ese proyecto
    const horasTrabajadasPorProyecto = [4, 9, 11, 2, 15, 7];

    console.log(Math.max(...horasTrabajadasPorProyecto))
